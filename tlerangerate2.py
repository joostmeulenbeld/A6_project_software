...
def compare(exprangerate):
    ...
    print exprangerate[1]
    
    dx = 5
    dy = []
    slope = []
    for i in range(1,len(exprangerate[1])):
        dy.append(exprangerate[1][i]-exprangerate[1][i-1])
        slope.append(dy[i-1]/dx)
        
    
    plt.subplot(1,2,2)
    plt.plot(exprangerate[0][1:],slope)
    plt.xlim(700,900)
    plt.ylim(-0.2,0.2)
    plt.show()
    ...
    for i in range(0,len(exprangerate[0])):
        if exprangerate[0][i]==700:
            start=i
        elif exprangerate[0][i]==900:
            end=i
